...
int v_agex_ld_reg, v_mem_ld_reg;
int v_agex_ld_cc, v_mem_ld_cc;
...
void MEM_stage() {
  ...
  int mem_w0,mem_w1;
  int mar0 = PS.MEM_ADDRESS & 0x0001; //MAR[0]
  int dcache_r;
  int write_word;
  int read_word;
  int from_memory;
  int ir11_9 = (PS.MEM_IR >> 9) & 0x0007;
  int sr_v = 0;
  ...
  else
  {
    ...
    write_word = (write_word << 8 ) + write_word;
  }  

 /*
  Control Logic to generate MEM_PCMUX
  */
 ...
 if(PS.MEM_V)
 {
   ...
   {
     if(((PS.MEM_CC >> 2) & N ) |( ((PS.MEM_CC >> 1 & 0x0001)) & Z) | ((PS.MEM_CC & 0x0001) & P))
      MEM_PCMUX = 1;
   }
 ...
 /*
  Control Logic for Dependency Logic 
  */
  ...
  if(Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V)
  {
    ...
    read_word = 0x0000;
        ...
        if(Get_DATA_SIZE(PS.MEM_CS) == 1 && mar0 == 0)
        from_memory = read_word;

        if(Get_DATA_SIZE(PS.MEM_CS) == 0 && mar0 == 1)
        {
          from_memory = signex(read_word >> 8, 8 , 16);
          
        }

        if(Get_DATA_SIZE(PS.MEM_CS) == 0 && mar0 == 0)
        {
          from_memory = signex(read_word - (read_word >>8<<8), 8 , 16);
        }
        
       
  mem_stall = 0;

  mem_stall =  !dcache_r & (Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V);

  if(mem_stall == 1)
  sr_v = 0;
 else if((mem_stall == 0) && PS.MEM_V)
  sr_v = 1;
  ...
  {
  //NEW_PS.SR_DATA = ((from_memory & 0x00FF) << 8) + (from_memory & 0x00FF);
...
void AGEX_stage() {
  ...
  int mem_address, mem_alu_result;
  int from_alu, from_shifter, from_lftshift1;
  int from_sr2mux;
  int ir = PS.AGEX_IR;
  ...
  int from_addr1mux, from_addr2mux, from_address_adder;

  if(Get_ADDR1MUX(PS.AGEX_CS))
    from_addr1mux = PS.AGEX_SR1;
  else
    from_addr1mux = PS.AGEX_NPC;

  if(Get_ADDR2MUX(PS.AGEX_CS) == 0)
    from_addr2mux = 0;
  if(Get_ADDR2MUX(PS.AGEX_CS) == 1)
    from_addr2mux = signex(ir - (ir>>6<<6), 6 , 16);
	if(Get_ADDR2MUX(PS.AGEX_CS) == 2)
			from_addr2mux = signex(ir - (ir>>9<<9), 9 , 16);
	if(Get_ADDR2MUX(PS.AGEX_CS) == 3)
			from_addr2mux = signex(ir - (ir>>11<<11), 11, 16);  

  if(Get_LSHF1(PS.AGEX_CS))
			from_address_adder = Low16bits((from_addr2mux<<1) + from_addr1mux);
		else 
			from_address_adder = Low16bits(from_addr2mux + from_addr1mux);

  if(Get_SR2MUX(PS.AGEX_CS))
    from_sr2mux = signex(PS.AGEX_IR - (PS.AGEX_IR >> 5 << 5), 5 ,16);
  else
    from_sr2mux = PS.AGEX_SR2;

  if(Get_ALUK(PS.AGEX_CS) == 0)
  from_alu =  Low16bits(PS.AGEX_SR1 + from_sr2mux);
  if(Get_ALUK(PS.AGEX_CS) == 1)
  from_alu =  PS.AGEX_SR1 & from_sr2mux;
  if(Get_ALUK(PS.AGEX_CS) == 2)
  from_alu =  PS.AGEX_SR1 ^ from_sr2mux;
  if(Get_ALUK(PS.AGEX_CS) == 3)
  from_alu = PS.AGEX_SR1;

  if(d == 0)
				from_shifter = Low16bits(PS.AGEX_SR1 << amount4);
			else
			{
				if(a)
				{
					if(PS.AGEX_SR1>>15 == 0)
							from_shifter = PS.AGEX_SR1 >> amount4;
					else
					{
						int sr1 = PS.AGEX_SR1;
						while(amount4 > 0)
  ...
  mem_alu_result = from_alu;
  else
  mem_alu_result = from_shifter;
  
  if(Get_ADDRESSMUX(PS.AGEX_CS) == 0)
  mem_address = (ir - (ir>>8<<8))<<1;

  else
  mem_address = from_address_adder;
  /*
  Control Logic for LD_MEM
  */
   int mem_v = PS.AGEX_V;
if(mem_stall)
  LD_MEM = 0;
 ...
 LD_MEM = 1; 
  /*
  Control Logic for Previous Stages
  */
...
int instruction = PS.DE_IR;

int ir15_11 = instruction >> 11;
int ir5 = (instruction  >> 5) & 0x0001;
int ir_reg = (instruction >> 6) & 0x0007;
int ir11_9 = (instruction >> 9) & 0x0007;
int ir2_0 = instruction & 0x0007;
int ir13 =  (instruction >> 13) & 0x0001; 
int opcode = instruction >> 12;
int drid;
int n,z,p;

int sr2_out;
int sr1_out = REGS[ir_reg];
...
CONTROL_STORE_ADDRESS = (ir15_11 << 1) + ir5;
...
if(PS.DE_V)
{
  ...
  {
    if(((PS.AGEX_DRID == ir_reg) && v_agex_ld_reg ) || ((PS.MEM_DRID == ir_reg) && v_mem_ld_reg) || ((PS.SR_DRID == ir_reg) && v_sr_ld_reg)  )
      dep_stall = 1;      
  }

  if(Get_SR2_NEEDED(DE_CS))
  {
    if(((PS.AGEX_DRID == drid) && v_agex_ld_reg ) || ((PS.MEM_DRID == drid) && v_mem_ld_reg) || ((PS.SR_DRID == drid) && v_sr_ld_reg)  )
      dep_stall = 1;      
  }

   if(Get_DE_BR_OP(DE_CS))
   {
     if(v_sr_ld_cc || v_agex_ld_cc || v_mem_ld_cc)
      dep_stall = 1;
   } 
}
...
Control Logic to generate LD_AGEX and input to AGEX.V

*/
int agex_v;
agex_v = !dep_stall & PS.DE_V; 

  v_de_br_stall = 0;

  if(PS.DE_V && Get_DE_BR_STALL(DE_CS))
      v_de_br_stall = 1;
...
else
{
  ...
  if (LD_AGEX) {
    /* Your code for latching into AGEX latches goes here */
      NEW_PS.AGEX_SR1 = sr1_out;
      NEW_PS.AGEX_SR2 = sr2_out; 
      NEW_PS.AGEX_CC = cc;

      if(Get_DRMUX(DE_CS) == 0)
       NEW_PS.AGEX_DRID = ir11_9;
      else 
      NEW_PS.AGEX_DRID = 7;  
...
void FETCH_stage() {
  ...
  icache_access(PC, &instruction, &icache_r);
   ...
   /*
        Control logic to generate ld_pc and ld_de 
    */

   int stall_pc =   !icache_r | v_de_br_stall | v_agex_br_stall | dep_stall | mem_stall | v_mem_br_stall;
   int stall_de_v = !icache_r | v_de_br_stall | v_agex_br_stall | v_mem_br_stall;
    ...
    // TRAP.PC and TARGET PC should be latched regardless of the br_stall
